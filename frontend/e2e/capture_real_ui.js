...
(async () => {
    ...
    for (const item of filteredMap) {
        ...
        if (currentRole !== targetRole) {
            ...
            if (targetRole) {
                ...
                let email = '';
                if (targetRole === 'admin') email = 'admin@demo.com';
                if (targetRole === 'staff') email = 'admin@demo.com'; // Use admin for staff flows since staff is not in seed
                if (targetRole === 'gv') email = 'giangvien@demo.com';
                if (targetRole === 'sv') email = 'sinhvien1@demo.com';
        ...
        if (type === 'form' || type === 'login' || type === 'dialog_error') {
            await page.evaluate(() => {
                ...
                inputs.forEach(input => {
                    if (!input.value) {
                        if (input.type === 'email') input.value = 'admin@demo.com';
        ...
        if (type.startsWith('dialog_')) {
            ...
            await page.evaluate(({realMessage, type}) => {
                ...
                popup.style.cssText = 'background:#fff;padding:30px;border:1px solid #ccc;width:450px;text-align:center;border-radius:12px;box-shadow: 0 10px 25px rgba(0,0,0,0.1); position: relative;';
                ...
                let btns = '<button style="padding:10px 25px;border:none;background:#2563eb;color:white;font-weight:600;margin-top:20px;border-radius:6px;cursor:pointer;">Đóng</button>';
                if (type === 'dialog_confirm') {
                    btns = '<button style="padding:10px 25px;border:1px solid #e5e7eb;background:#fff;margin-right:15px;color:#374151;font-weight:600;border-radius:6px;cursor:pointer;">Hủy bỏ</button>' + btns;
                }

                popup.innerHTML = `
                    <h3 style="margin-top:0;font-size:20px;font-weight:600;color:#111827;">${icon} ${header}</h3>
                    <p style="font-size:15px;color:#4b5563;margin:20px 0;">${realMessage}</p>
    ...
    await browser.close();
    console.log("Done generating Real UI mockups!");
})();
