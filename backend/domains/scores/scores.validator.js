...
const validateScoreSheetSubmit = (req, res, next) => {
  ...
  if (!rubricRole || !['SUPERVISOR', 'SECOND_MARKER', 'RECHECK'].includes(rubricRole)) {
    errors.push({ field: 'rubricRole', code: 'RUBRIC_ROLE_INVALID', message: 'Loại phiếu chấm phải là một trong: SUPERVISOR, SECOND_MARKER, RECHECK.' });
  }

  if (!targetType || !['SUPERVISOR', 'SECOND_MARKER', 'RECHECK'].includes(targetType)) {
